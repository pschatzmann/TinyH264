...
static int compareRegion(const char* name, const uint8_t* got, int gotStride,
  ...
  if (mismatches > 0) {
    printf("%s: %d/%d pixels mismatch (first at x=%d,y=%d got=%d want=%d, "
           ...
           firstX / (w==176?16:8), firstY / (w==176?16:8));
  } else {
...
#ifdef DUMP_MB
  {
    int mbx = DUMP_MB_X, mby = DUMP_MB_Y;
    printf("--- MB(%d,%d) got vs want (Y) ---\n", mbx, mby);
    for (int y = 0; y < 16; y++) {
      for (int x = 0; x < 16; x++)
        printf("%3d ", f.y()[(mby * 16 + y) * f.strideY + mbx * 16 + x]);
      printf("  | ");
      for (int x = 0; x < 16; x++)
        printf("%3d ", refY[(mby * 16 + y) * 176 + mbx * 16 + x]);
      printf("\n");
    }
  }
#endif
...
#ifdef DUMP_MB_LIST
  {
    printf("--- mismatching MBs (Y) ---\n");
    for (int mby = 0; mby < 9; mby++) {
      for (int mbx = 0; mbx < 11; mbx++) {
        int cnt = 0;
        for (int y = 0; y < 16; y++)
          for (int x = 0; x < 16; x++) {
            int ay = mby * 16 + y, ax = mbx * 16 + x;
            if (f.y()[ay * f.strideY + ax] != refY[ay * 176 + ax]) cnt++;
          }
        printf("%4d", cnt);
      }
      printf("\n");
...
#ifdef DUMP_MB_CHROMA
  {
    int mbx = DUMP_MB_X, mby = DUMP_MB_Y;
    printf("--- MB(%d,%d) got vs want (U) ---\n", mbx, mby);
    for (int y = 0; y < 8; y++) {
      for (int x = 0; x < 8; x++)
        printf("%3d ", f.u()[(mby * 8 + y) * f.strideC + mbx * 8 + x]);
      printf("  | ");
      for (int x = 0; x < 8; x++)
        printf("%3d ", refU[(mby * 8 + y) * 88 + mbx * 8 + x]);
      printf("\n");
    }
  }
#endif

#ifdef DUMP_MB_LIST_CHROMA
  {
    printf("--- mismatching MBs (U) ---\n");
    for (int mby = 0; mby < 9; mby++) {
      for (int mbx = 0; mbx < 11; mbx++) {
        int cnt = 0;
        for (int y = 0; y < 8; y++)
          for (int x = 0; x < 8; x++) {
            int ay = mby * 8 + y, ax = mbx * 8 + x;
            if (f.u()[ay * f.strideC + ax] != refU[ay * 88 + ax]) cnt++;
          }
        printf("%4d", cnt);
      }
      printf("\n");
